...
function showmap()
  ...
  while box_x1 < 260 do
    ...
    while box_y1 < 230 do
	  ...
	  local stuff = 2*bit.rshift(camx + box_x1 + 1, 4) + memory.read_u16_le(0x0829 + 2*bit.rshift(camy + box_y1 + 1, 4))
	  local stuffand = bit.band(memory.read_u16_le(0x2000+stuff), 0x03FF)
	  local weird = bit.band(memory.read_u16_le(0x2000+stuff), 0xFC00)
	  local incollision = memory.read_u16_le(0xF400 + stuffand);
...
local my_draw = function()
  ...
  local subposx = memory.read_u32_le(0x005A);
  ...
  -- Draw Mr.Nutz hitbox
  gui.drawBox(hitbox_x1, hitbox_y1, hitbox_x2, hitbox_y2, Mrnut_hitbox_color_outline, Mrnut_hitbox_color )
  -- Draw Mr. Nutz x/y "center"

  --gui.drawLine(posx - 2 - camx, posy - camy, posx + 2 - camx, posy - camy, 0xAAFF0000)
 --gui.drawLine(posx - camx , posy - 2 - camy, posx - camx, posy + 2 - camy, 0xAAFF0000)
